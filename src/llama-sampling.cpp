...
void llama_sample_entropy_impl(struct llama_sampling * smpl, llama_token_data_array * candidates, float min_temp, float max_temp, float exponent_val, float smoothing_factor) {
    ...
    if (smoothing_factor > 0 && candidates->size > 1) {

        llama_sample_softmax_impl((struct llama_sampling *) nullptr, candidates);
        float h = candidates->data[0].logit; // Find the maximum logit for h to be added after the transformation
        // Apply quadratic transformation using the smoothing_factor
        for (size_t i = 0; i < candidates->size; ++i)
        {
            float logit_shifted = candidates->data[i].logit - h;
            candidates->data[i].logit = -smoothing_factor * logit_shifted * logit_shifted + h;
        }
        llama_sample_softmax_impl((struct llama_sampling *) nullptr, candidates);
    }
